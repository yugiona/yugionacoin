...
const char * const BITCOIN_CONF_FILENAME = "yugionacoin.conf";
const char * const BITCOIN_PID_FILENAME = "yugionacoin.pid";
...
#else
    const char* pszModule = "yugionacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Yugionacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Yugionacoin";
#else
    // Unix
    return pathRet / ".yugionacoin";
#endif
#endif
}
